...
- [M4 本機人工驗收手冊](docs/manual-test-m4.md)
...
5. 確認／建立客戶、選地址／設備／負責人並按「分流案件」。
6. 按「建立／查看正式報價」，輸入品項、數量、客戶單價、成本、稅率與條款，先儲存草稿。
7. owner／admin 勾選人工覆核後按「核准並建立分享連結」。目前 M6 尚未接 LINE 自動發送，請把這條真實安全連結貼到原本的 LINE 對話。
8. 用無痕視窗開報價連結；客戶不需註冊，填確認人姓名，選接受或拒絕，再做第二次確認。
9. owner 重新整理報價頁：接受後可回進件轉成單次工單／專案；拒絕後可複製為 v2 草稿，v1 仍不可修改。
10. 重新整理各頁；進件、報價版本、客戶決定、內部備註與案件編號都應從 PostgreSQL 讀回。

目前完成到 M4：從公開進件、人工分流、正式報價、客戶免註冊接受／拒絕，到接受後轉案件都是真資料。派工／技師完工頁（M5）與 LINE webhook／自動通知（M6）仍未完成；因此現階段需手動把安全連結貼入 LINE，也不會顯示尚未存在的案件工作台死連結。
