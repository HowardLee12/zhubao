...
function NewQuoteContent() {
  ...
  const [loadingClone, setLoadingClone] = useState(!!cloneFrom);

  const loadCloneData = useCallback(async () => {
    if (!cloneFrom) return;
    try {
      const res = await fetch(`/api/quotes/${cloneFrom}`);
      if (!res.ok) {
  ...
  if (!projectId) {
    return (
      <div className="p-4 text-center text-muted-foreground">
        ...
        <div className="text-sm">請從案件頁面建立報價單</div>
      </div>
    );
  }

  if (loadingClone) {
    return <div className="p-8 text-center text-muted-foreground text-sm">載入報價資料中...</div>;
  }
  ...
  return (
    <div>
      ...
      {/* Profit summary (cost mode only) */}
      {mode === "cost" && sections.some((s) => s.items.length > 0) && (
              ...
              <div key={item.id} className="px-4 py-3 border-b border-sage-50 last:border-0">
                ...
                <div className="grid grid-cols-4 gap-2">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => updateItem(section.id, item.id, "markupPercent", Number(e.target.value))}
                      min={0}
                      max={200}
                      className="w-full px-2 py-1.5 rounded-lg border border-border text-xs focus:outline-none focus:ring-1 focus:ring-primary/30"
                    />
                ...
                <div className="flex justify-between mt-2 text-[11px]">
                  <span className="text-muted-foreground">
                    成本 {formatCurrency(item.unitCost * item.quantity)}
                  </span>
...
export default function NewQuotePage() {
  return (
    <Suspense fallback={<div className="p-4 text-center text-muted-foreground text-sm">載入中...</div>}>
      <NewQuoteContent />
    </Suspense>
  );
}
