...
import { assetRowSchema, toAssetDto } from "@/schemas/asset";
...
export async function GET(request: Request, context: RouteContext): Promise<Response> {
  ...
  try {
    ...
    const { data, error } = await supabase.rpc("list_pilot_customer_assets", {
      p_organization_id: parsedOrgId.data,
      p_customer_id: parsedCustomerId.data,
    });
    if (error) throw mapServiceRequestRpcError(error);

    const rows = z.array(assetRowSchema).safeParse(data ?? []);
    if (!rows.success) throw internalApiProblem();

    return apiJsonResponse({ data: rows.data.map((row) => toAssetDto(row)) }, { requestId });
  } catch (error) {
