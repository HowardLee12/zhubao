...
export async function GET(request: Request, context: RouteContext): Promise<Response> {
  ...
  try {
    ...
    const { data, error } = await supabase.rpc("get_pilot_quote_workspace_by_request", {
      p_organization_id: ids.data.orgId,
      p_service_request_id: ids.data.id,
    });
    ...
    return apiJsonResponse(
      { data: workspace.data },
      { requestId, headers: { etag: `"${workspace.data.quote.lockVersion}"` } },
    );
  } catch (error) {
