...
/**
 * Resolve the `Idempotency-Key` for a staff mutation whose client does not always
 * send one (e.g. the pilot team-invite form). A supplied key is validated and
 * honoured so a genuine retry replays; when absent, a fresh random key is minted
 * so the request still flows through the RPC's idempotency ledger. Uniqueness of
 * the created resource is additionally guaranteed by the underlying DB constraint,
 * so a missing header can never cause a duplicate.
 */
export function resolveStaffIdempotencyKey(value: string | null | undefined): string {
  if (!value) {
    return randomUUID();
  }
